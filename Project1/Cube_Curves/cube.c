...
int cube_search(int *h_quer, struct list_node ***f, struct list_node **cube, struct curve *curves, struct curve query, double *distanceCube, int vec_sum, int coords, int d, int probes, int M){
	int i, j, hash_pos, vec_pos, min_pos, *cube_pos, *binary_string, count;
	double dist, min;
	struct list_node *cur;
